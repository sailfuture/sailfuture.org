...
async function FeaturedPosts() {
  ...
  return (
    <div className="mt-6 bg-gradient-to-t from-gray-100 pb-6 z-20 relative">
      <Container>
        <h2 className="text-2xl font-medium tracking-tight z-20 relative">Featured</h2>
        <div className="mt-3 grid grid-cols-1 gap-8 lg:grid-cols-3 z-20">
          {featuredPosts.map((post) => (
            <div
              key={post.slug}
              className="relative flex flex-col rounded-3xl bg-white p-2 shadow-md shadow-black/5 ring-1 ring-black/5 z-20"
            >
              {post.mainImage && (
                <img
                  alt={post.mainImage.alt || ''}
                  src={image(post.mainImage).size(1170, 780).url()}
                  className="aspect-[3/2] w-full rounded-2xl object-cover z-20"
                />
              )}
    ...
    <div className="mt-6 z-20 relative">
      {posts.map((post) => (
        ...
        >
          ...
          <div className="sm:col-span-2 sm:max-w-2xl relative">
            <h2 className="text font-medium relative">{post.title}</h2>
            <p className="mt-3 text-gray-500 relative">{post.excerpt}</p>
            <div className="mt-4">
              <Link
                href={`/blog/${post.slug}`}
                className="flex items-center gap-1 text-sm/5 font-medium relative"
              >
                <span className="absolute inset-0" />
...
async function Pagination({ page, category }) {
  ...
  return (
    <div className="mt-6 flex items-center justify-between gap-2 z-80 relative bg=white">
      <Button
        variant="outline"
...
export default async function Blog({ searchParams }) {
  ...
  return (
    <main className="overflow-hidden relative">
      ...
      <Container>
      <Heading as="h1" className="mt-6 relative z-20">
        Recent News
        </Heading>
      </Container>
      {page === 1 && !category && <FeaturedPosts />}
      <Container className="mt-16 pb-24 relative z-80">
